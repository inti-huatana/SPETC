...
depth, read noise and dark current; a \textbf{Load QE CSV} button that reads
...
(\code{parallactic} or \code{fixed}), and the grating efficiency. A
\emph{single} \textbf{grating lines/mm} field feeds both the slitless
disperser and the Littrow geometry helper (the previous duplicate field is
gone). Grating efficiency can be either a scalar 0--1 or a
wavelength-dependent curve loaded from a two-column CSV with
\textsf{Load CSV\ldots} (the curve overrides the scalar; \textsf{Clear}
reverts). The \textbf{spectrograph geometry helper} takes the
