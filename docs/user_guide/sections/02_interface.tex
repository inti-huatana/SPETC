...
obstruction, focal length, scalar optics throughput (excluding QE), and an
optional calibrated instrument-response curve with its wavelength unit. The
...
the QE wavelength unit; the optional calibrated slit resolving-power curve;
the \textbf{sensor type} — \code{monochrome} for a mono CCD/CMOS, or
\code{color} for a one-shot-colour (Bayer) sensor read one channel at a
time, with the R/G/B channel selector (greyed out for a monochrome sensor);
aperture rates and pixel counts scale by the channel's Bayer fill fraction,
saturation stays per physical pixel, and the QE curve should be that
channel's effective QE (sensor QE $\times$ colour-filter dye). At the end of
...
the slitless parameters (Sect.~\ref{sec:spec-guide}), the \textbf{WL
min/max/ref} on one row with an \textbf{AA/nm/um} unit selector, the
\textbf{slit orientation} (\code{parallactic} or \code{fixed}), and the
grating efficiency. The fields of the mode you are not using are greyed
