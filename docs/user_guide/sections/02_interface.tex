...
as a CSV in \code{data/horizons/} (format in
Sect.~\ref{sec:file-horizon}). \textsf{Display horizon} plots the saved
profile for the current coordinates at any time. The generator needs two
...
template's synthetic colours against the catalogue. Two optional fields
transform the template before it is calibrated: the \textbf{radial
velocity} in km/s (positive receding) shifts all wavelengths by
$(1+v/c)$ — relevant for high-velocity stars and extragalactic targets
at low resolution — and \textbf{E(B$-$V)} reddens the template with the
