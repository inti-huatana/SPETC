...
Press \textsf{Calculate defocused PSF}. The geometric image of the star is
an annulus --- a ``donut'' --- whose outer radius on the focal plane is
$r_{\rm out} = \delta D / (2F)$ for a defocus $\delta$, aperture diameter
$D$ and focal length $F$; the central obstruction punches a hole of radius
$r_{\rm in} = \varepsilon\,r_{\rm out}$ with
$\varepsilon = $ (obstruction diameter)/$D$ (a classical
Ritchey--Chr\'etien or Cassegrain when the obstruction is non-zero, a
filled disc when it is 0). Crucially, the program does \emph{not} stop at
that idealised flat annulus with vertical edges --- which no camera ever
records --- but \textbf{convolves it with the atmospheric PSF} you
selected (the seeing FWHM and Gaussian/Moffat model from the conditions
box, plus any guiding rms). The displayed profile therefore has the
rounded rims, partly-filled centre and wings of a real defocused star, and
a small defocus (a donut comparable to or smaller than the seeing)
correctly reduces to a merely broadened stellar core rather than a ring.
The window shows two panels: the radial intensity cross-section (a
symmetric cut from $-r$ to $+r$, normalised to its peak, with the ideal
geometric edges marked as dotted lines for reference) and the cumulative
encircled energy versus radius. Both the figure and the underlying tables
(radius in pixels, radius in arcsec, and either the normalised intensity
or the encircled energy in per cent) are saved into the session directory
with \textsf{Save figure + data}.
...
The model already includes the dominant blur --- the seeing (and guiding)
convolution above. What it still neglects is diffraction ringing at the
donut edges and optical aberrations (spherical, coma; small on-axis for a
paraboloid or an RC); both are sub-dominant once the donut is much larger
than the seeing and diffraction scales, which is the regime defocused
photometry is used in.
