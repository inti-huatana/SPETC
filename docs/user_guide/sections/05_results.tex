...
\begin{itemize}
  ...
  \item \textbf{Time series}: one row per time sample with UTC and local
        ...
        electrons and maximum unsaturated exposure, plus --- for both
        photometry and spectroscopy --- the \textbf{total photometric
        error} and the \textbf{scintillation error}, each in milli-mag, at
        every time. This table is the CSV export.
  ...
  \item \textbf{Outputs}: the selected-time results, in a tab of their own:
        S/N, scintillation noise in electrons, as a percentage of the
        source and \textbf{in milli-mag}, the \textbf{total photometric
        error in milli-mag} ($1085.7 / (\text{S/N})$), the \textbf{Airy
        disk size} (diameter $2.44\,\lambda/D$) in arcsec and in pixels,
        ADC noise, peak-pixel and saturation numbers, a
        \textbf{frame-count summary} (how many of the sampled frames are
        linear, non-linear or saturated), the aperture radius in arcsec ---
        and, in \code{defocus} geometry, the focus position and the donut
        encircled-energy fraction captured in that aperture --- predicted
