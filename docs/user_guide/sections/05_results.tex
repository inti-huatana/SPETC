...
\begin{itemize}
  ...
  \item \textbf{Time series}: one row per time sample with UTC and local
        ...
        export.
  \item \textbf{Assumptions / outputs}: a plain-language statement of
        every physical assumption of the current run --- sky model and its
        components (including the SQM value in \code{sqm} mode), PSF
        model, the CMOS gain-table setting in use, the spectrograph
        geometry, noise terms, saturation policy --- plus the
        selected-time altitude, airmass, parallactic angle and sky
        brightness, followed by a \textbf{SELECTED-TIME OUTPUTS} block:
        ...
        magnitudes, the $\sigma$(EW) line criterion, the stack plan and
        the differential-photometry precision.
\end{itemize}
