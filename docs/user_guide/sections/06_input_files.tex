...
\subsection{Sites: \code{etc\_sites.json} and \code{observatories.json}}
...
\code{elev} metres, \code{sky\_ab\_mag\_arcsec2} the default value for the
fixed-sky mode. You rarely edit this file by hand: \textsf{Save current
site} writes the entry for you, and \textsf{Import site file} merges
another user's file into yours. \code{observatories.json} holds the
shipped defaults in a simpler flat format and is not meant to be edited.
